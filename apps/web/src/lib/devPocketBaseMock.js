...
/**
 * Auth that treats any stored session as valid (no JWT expiry checks) for local UI testing.
 */
...
export function createDevMockPocketBase() {
	...
	const client = new PocketBase('/hcgi/platform', new DevAuthStore());
	...
	client.send = async function devSend(path, options) {
		...
		console.warn('[devPocketBaseMock] unhandled request:', method, pathOnly);
		throw new ClientResponseError({
			status: 501,
	...
	if (!client.authStore.isValid) {
		client.authStore.save('dev_mock_auto_session', { ...MOCK_USER_RECORD });
	}
