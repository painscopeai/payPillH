 ...
 * Keeps this route OFF the heavy Express bundle (api/index.mjs) to avoid cold-start timeouts on Vercel.
 */
...
export default async function handler(req, res) {
	...
	fetch('http://127.0.0.1:7835/ingest/ac6048b3-2d29-4ab3-ac92-730ceeebf184', {
		...
		body: JSON.stringify({
			...
			hypothesisId: 'H1',
			location: 'api/health/patient-dashboard-metrics.mjs:entry',
	...
	fetch('http://127.0.0.1:7835/ingest/ac6048b3-2d29-4ab3-ac92-730ceeebf184', {
		...
		body: JSON.stringify({
			...
			runId: process.env.VERCEL === '1' ? 'prod' : 'pre-fix',
			hypothesisId: 'H2',
			location: 'api/health/patient-dashboard-metrics.mjs:after_getUser',
			message: 'auth_ms',
	...
	try {
		const { inferDashboardMetrics } = await import('../../apps/api/src/health-risk/inferDashboardMetrics.js');
		// #region agent log
		fetch('http://127.0.0.1:7835/ingest/ac6048b3-2d29-4ab3-ac92-730ceeebf184', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': 'a604a1' },
			body: JSON.stringify({
				...
				runId: process.env.VERCEL === '1' ? 'prod' : 'pre-fix',
				hypothesisId: 'H4',
				location: 'api/health/patient-dashboard-metrics.mjs:before_infer',
		...
		fetch('http://127.0.0.1:7835/ingest/ac6048b3-2d29-4ab3-ac92-730ceeebf184', {
			...
			body: JSON.stringify({
				...
				runId: process.env.VERCEL === '1' ? 'prod' : 'pre-fix',
				hypothesisId: 'H3',
				location: 'api/health/patient-dashboard-metrics.mjs:success',
		...
		fetch('http://127.0.0.1:7835/ingest/ac6048b3-2d29-4ab3-ac92-730ceeebf184', {
			...
			body: JSON.stringify({
				...
				runId: process.env.VERCEL === '1' ? 'prod' : 'pre-fix',
				hypothesisId: 'H3',
				location: 'api/health/patient-dashboard-metrics.mjs:error',
